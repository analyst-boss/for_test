...
m_func.show_notification_windows(title = name_excel_for_update,
                                 message = 'Начало работы "2_check_update_excel_and_mail.py"'
                                 )
...
m_func.log(new_log_path, f'Работа скрипта "2_check_update_excel_and_mail.py" за дату: {today}')
...
try:
    ...
    # если текущая дата НЕ равна последнему обновлению Excel -> то обовлять Excel нужно -> идем искать письмо
    else:
    
        m_func.log(new_log_path, f'Excel сегодня HE обновлялся.')
        m_func.log(new_log_path, f'Идет поиск письма.')

        # название папки, в которой нужно искать письмо
        mail_folder = config.alert_folder

        # функция для поиска папки в Outlook
        check_folder_outlook = m_func.find_folder_outlook(mail_folder, new_log_path)

        # проверяем наличие папки с алертами
        if check_folder_outlook is None:
        
            m_func.log(new_log_path, f'Папка "{mail_folder}" НЕ нашлась.\nНа всякий случай, проверь название папки в почтовом ящике и название в переменной config.alert_folder.')

            # отправляем письмо
            m_func.send_mail_outlook(recipient_mail = config.recipient_mail,
                                    subject_mail = config.theme_mail,
                                    body_mail = f'Папка "{mail_folder}" НЕ нашлась. Читай файл с логами.\nНа всякий случай, проверь название папки в почтовом ящике и название в переменной config.alert_folder.',
                                    attach_files = [new_log_path],
                                    account_outlook = config.account_outlook.lower(),
                                    new_log_path = new_log_path
                                    )
            ...
            # переменная со всеми письмами в папке Outlook
            messages = check_folder_outlook.Items

            # текст, который нужно искать в письме
            text_in_mail = config.find_text_in_mail

            # функция проверяет наличие письма в папке Outlook
            found_mail = m_func.find_mail_outlook(text_in_mail, messages, new_log_path)

            # если письмо нашлось
            if found_mail is True:

                m_func.log(new_log_path, f'Письмо нашлось.')
            ...
            # если письмо НЕ нашлось
            else:

                m_func.log(new_log_path, f'Письмо НЕ нашлось. На всякий случай проверь паттерн поиска письма.\n')
                
                # отправлять уведомление по почте, когда письмо НЕ нашлось 
                # если переменная config.send_mail_if_mail_no_found is False -> то письмо не отправится
                if config.send_mail_if_mail_no_found is True:
                                       
                    # отправляем письмо
                    m_func.send_mail_outlook(recipient_mail = config.recipient_mail,
                                            subject_mail = config.theme_mail,
                                            body_mail = f'Письмо НЕ нашлось. На всякий случай проверь паттерн поиска письма.',
                                            attach_files = [new_log_path],
                                            account_outlook = config.account_outlook.lower(),
                                            new_log_path = new_log_path
                                            )
    ...
    m_func.log(new_log_path, f'Ошибка при отработке скрипта "2_check_update_excel_and_mail.py": {str(e)}\n')

    # отправляем письмо
    m_func.send_mail_outlook(recipient_mail = config.recipient_mail,
                             subject_mail = config.theme_mail,
                             body_mail = f'Excel "{name_excel_for_update}" HE обновился.\nОшибка при отработке скрипта "2_check_update_excel_and_mail.py": {str(e)}.\nЧитай файл с логами.',
                             attach_files = [new_log_path],
                             account_outlook = config.account_outlook.lower(),
                             new_log_path = new_log_path
                             )
    ...
    # системное уведомление windows
    m_func.show_notification_windows(title = name_excel_for_update,
                                     message = 'Конец работы "2_check_update_excel_and_mail.py"'
                                     )